...
struct DateWidgetSettings: View {
    @State private var format: String
    @State private var showNextEventInBar: Bool
    @State private var showNextEventInPopup: Bool

    init() {
        _format = State(initialValue: DateSettings.shared.format)
        _showNextEventInBar = State(initialValue: DateSettings.shared.showNextEventInBar)
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Toggle("Show on Bar", isOn: $showNextEventInBar)
                .onChange(of: showNextEventInBar) { _, newValue in
                    DateSettings.shared.showNextEventInBar = newValue
                }
